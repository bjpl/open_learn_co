"""
Dinero magazine scraper implementation
Colombia's leading business and financial magazine
"""

import re
from typing import Dict, List, Optional
from datetime import datetime
from bs4 import BeautifulSoup
import logging

from scrapers.base.smart_scraper import SmartScraper, ScrapedDocument

logger = logging.getLogger(__name__)


class DineroScraper(SmartScraper):
    """Scraper for Dinero magazine (dinero.com)"""

    def __init__(self, source_config: Dict = None):
        config = source_config or {
            'name': 'Dinero',
            'url': 'https://www.dinero.com',
            'rate_limit': '8/minute'
        }
        super().__init__(config)

        self.sections = [
            'economia',
            'empresas',
            'finanzas',
            'inversion',
            'emprendimiento',
            'tecnologia',
            'opinion',
            'pais'
        ]

        # CSS selectors for Dinero's structure
        self.selectors = {
            'article_links': '.article-link, .story-link, .card-link, .headline-link, h2 a, h3 a',
            'title': 'h1.article-title, h1.story-title, h1.headline, h1',
            'subtitle': '.article-subtitle, .story-subtitle, .bajada, .dek',
            'content': '.article-content, .story-content, .content-body, .article-text',
            'author': '.article-author, .story-author, .byline, .autor',
            'date': '.article-date, .story-date, time, .publish-date',
            'category': '.article-category, .story-category, .section',
            'tags': '.article-tags a, .story-tags a, .tags a'
        }

    def extract_article_urls(self, homepage_html: str) -> List[str]:
        """Extract article URLs from Dinero homepage"""
        soup = self.parse_html(homepage_html)
        urls = set()

        # Extract from homepage
        article_links = soup.select(self.selectors['article_links'])

        for link in article_links:
            href = link.get('href', '')
            if href:
                if href.startswith('/'):
                    href = self.base_url + href
                if self._is_article_url(href):
                    urls.add(href)

        # Get articles from business sections
        for section in self.sections[:6]:  # Focus on core business sections
            section_url = f"{self.base_url}/{section}"
            section_html = self.fetch_page(section_url)
            if section_html:
                section_soup = self.parse_html(section_html)
                section_links = section_soup.select(self.selectors['article_links'])

                for link in section_links:
                    href = link.get('href', '')
                    if href:
                        if href.startswith('/'):
                            href = self.base_url + href
                        if self._is_article_url(href):
                            urls.add(href)

        return list(urls)

    def _is_article_url(self, url: str) -> bool:
        """Check if URL is a valid Dinero article"""
        # Business article indicators
        article_indicators = [
            '/economia/',
            '/empresas/',
            '/finanzas/',
            '/inversion/',
            '/emprendimiento/',
            '/tecnologia/',
            '/pais/',
            '/articulo/',
            '/noticia/'
        ]

        # Exclude patterns
        exclude_patterns = [
            '/autor/',
            '/tag/',
            '/seccion/',
            '/multimedia/',
            '/video/',
            '/galeria/',
            '/podcast/',
            '/eventos/',
            '/ranking/',
            '/directorio/',
            '#',
            'mailto:',
            'javascript:',
            '.pdf',
            '.jpg',
            '.png',
            'twitter.com',
            'facebook.com',
            'linkedin.com'
        ]

        # Check indicators
        has_indicator = any(indicator in url.lower() for indicator in article_indicators)

        # Check exclusions
        should_exclude = any(pattern in url.lower() for pattern in exclude_patterns)

        # Check for article ID pattern
        has_id = bool(re.search(r'/\d{5,}', url))

        return (has_indicator or has_id) and not should_exclude

    def extract_article_content(self, article_html: str, url: str) -> Optional[ScrapedDocument]:
        """Extract content from Dinero article"""
        soup = self.parse_html(article_html)

        try:
            # Extract title
            title_elem = soup.select_one(self.selectors['title'])
            if not title_elem:
                logger.warning(f"No title found for {url}")
                return None
            title = self._clean_text(title_elem.get_text())

            # Extract subtitle
            subtitle_elem = soup.select_one(self.selectors['subtitle'])
            subtitle = self._clean_text(subtitle_elem.get_text()) if subtitle_elem else ""

            # Extract content
            content_elem = soup.select_one(self.selectors['content'])
            if not content_elem:
                logger.warning(f"No content found for {url}")
                return None

            # Process content
            content_parts = []
            if subtitle:
                content_parts.append(subtitle)

            paragraphs = content_elem.find_all(['p', 'div'])
            for p in paragraphs:
                text = self._clean_text(p.get_text())
                if text and len(text) > 30:
                    # Skip promotional content
                    if not self._is_promotional_content(text):
                        content_parts.append(text)

            content = ' '.join(content_parts)

            # Validate content
            if len(content) < 250:
                logger.warning(f"Content too short for {url}")
                return None

            # Check for paywall
            if self._is_paywall_content(content):
                logger.info(f"Paywall detected for {url}")

            # Extract metadata
            author = self._extract_author(soup)
            published_date = self._extract_date(soup, url)
            category = self._extract_category(soup, url)

            # Business magazine specific analysis
            entities = self._extract_business_entities(content)
            financial_data = self._extract_financial_data(content)
            business_type = self._classify_business_content(content, url)
            difficulty = self._calculate_magazine_difficulty(content)

            # Create document
            doc = ScrapedDocument(
                source=self.source_name,
                source_type='business_magazine',
                url=url,
                title=title,
                content=content,
                author=author,
                published_date=published_date,
                metadata={
                    'category': category,
                    'business_entities': entities,
                    'financial_data': financial_data,
                    'business_type': business_type,
                    'word_count': len(content.split()),
                    'source_focus': 'business_analysis',
                    'target_audience': 'executives',
                    'content_depth': 'deep_analysis'
                },
                language='es',
                difficulty_level=self._get_difficulty_level(difficulty),
                region='Colombia',
                categories=[category] if category else ['empresas']
            )

            return doc

        except Exception as e:
            logger.error(f"Error extracting content from {url}: {e}")
            return None

    def _is_promotional_content(self, text: str) -> bool:
        """Check if text is promotional content"""
        promo_indicators = [
            'suscríbete',
            'premium',
            'contenido patrocinado',
            'publicidad',
            'boletín',
            'newsletter',
            'descarga',
            'síguenos',
            'evento dinero',
            'ranking dinero'
        ]
        text_lower = text.lower()
        return any(indicator in text_lower for indicator in promo_indicators)

    def _is_paywall_content(self, content: str) -> bool:
        """Check if content is behind paywall"""
        paywall_indicators = [
            'contenido exclusivo',
            'suscríbete para',
            'acceso ilimitado',
            'contenido premium',
            'análisis completo'
        ]
        content_lower = content.lower()
        return any(indicator in content_lower for indicator in paywall_indicators)

    def _extract_author(self, soup: BeautifulSoup) -> Optional[str]:
        """Extract article author"""
        author_selectors = [
            '.article-author',
            '.story-author',
            '.byline',
            '.autor',
            '.redaccion'
        ]

        for selector in author_selectors:
            author_elem = soup.select_one(selector)
            if author_elem:
                author_text = self._clean_text(author_elem.get_text())
                # Clean prefixes
                author_text = re.sub(r'^(Por:?\s*|Redacción\s*|@)', '', author_text, flags=re.IGNORECASE)
                if author_text and len(author_text) > 2:
                    return author_text

        return "Dinero"

    def _extract_date(self, soup: BeautifulSoup, url: str) -> Optional[datetime]:
        """Extract publication date"""
        # Try time element
        time_elem = soup.select_one('time')
        if time_elem and time_elem.get('datetime'):
            try:
                return datetime.fromisoformat(time_elem.get('datetime').replace('Z', '+00:00'))
            except:
                pass

        # Try date selectors
        date_selectors = ['.article-date', '.story-date', '.publish-date', '.fecha']
        for selector in date_selectors:
            date_elem = soup.select_one(selector)
            if date_elem:
                date_text = self._clean_text(date_elem.get_text())
                parsed_date = self._parse_spanish_date(date_text)
                if parsed_date:
                    return parsed_date

        # Extract from URL
        date_match = re.search(r'/(\d{4})/(\d{2})/(\d{2})/', url)
        if date_match:
            year, month, day = date_match.groups()
            try:
                return datetime(int(year), int(month), int(day))
            except:
                pass

        return datetime.utcnow()

    def _extract_category(self, soup: BeautifulSoup, url: str) -> str:
        """Extract article category"""
        # Try category elements
        category_selectors = ['.article-category', '.story-category', '.section']
        for selector in category_selectors:
            cat_elem = soup.select_one(selector)
            if cat_elem:
                category = self._clean_text(cat_elem.get_text())
                if category and category.lower() not in ['inicio', 'home', 'dinero']:
                    return category

        # Extract from URL
        for section in self.sections:
            if f'/{section}/' in url.lower():
                section_map = {
                    'economia': 'Economía',
                    'empresas': 'Empresas',
                    'finanzas': 'Finanzas',
                    'inversion': 'Inversión',
                    'emprendimiento': 'Emprendimiento',
                    'tecnologia': 'Tecnología',
                    'opinion': 'Opinión',
                    'pais': 'País'
                }
                return section_map.get(section, section.title())

        return 'Empresas'

    def _extract_business_entities(self, text: str) -> Dict[str, List[str]]:
        """Extract business and corporate entities"""
        entities = {}

        patterns = {
            'colombian_corporations': [
                r'\b(Ecopetrol|Bancolombia|Grupo Éxito|EPM|ETB|ISA)\b',
                r'\b(Avianca|Grupo Sura|Grupo Bolívar|Nutresa|Corona)\b',
                r'\b(Argos|Bavaria|Alpina|Carvajal|Manuelita)\b'
            ],
            'financial_institutions': [
                r'\b(Banco de la República|Superfinanciera|BVC)\b',
                r'\b(BBVA Colombia|Davivienda|Banco de Bogotá)\b',
                r'\b(Credicorp Capital|Corficolombiana|Valores Bancolombia)\b'
            ],
            'international_companies': [
                r'\b(Amazon|Google|Microsoft|Apple|Tesla)\b',
                r'\b(JPMorgan|Goldman Sachs|BlackRock|Berkshire)\b',
                r'\b(Coca-Cola|Unilever|Nestlé|P&G)\b'
            ],
            'government_economic': [
                r'\b(MinHacienda|MinComercio|DANE|DNP|DIAN)\b',
                r'\b(Procolombia|Bancóldex|Finagro|FDN)\b',
                r'\b(SIC|Superintendencia de Sociedades)\b'
            ],
            'business_leaders': [
                r'\b(Carlos Ardila Lülle|Luis Carlos Sarmiento)\b',
                r'\b(Jaime Gilinski|Santo Domingo|David Bojanini)\b',
                r'\b(María Lorena Gutiérrez|Gonzalo Córdoba)\b'
            ],
            'economic_indicators': [
                r'\b(PIB|inflación|desempleo|IPC|DTF|TRM)\b',
                r'\b(riesgo país|grado de inversión|rating)\b',
                r'\b(EBITDA|ROE|ROA|margen operacional)\b'
            ]
        }

        for entity_type, pattern_list in patterns.items():
            found = set()
            for pattern in pattern_list:
                matches = re.findall(pattern, text, re.IGNORECASE)
                found.update(matches)
            if found:
                entities[entity_type] = list(found)

        return entities

    def _extract_financial_data(self, text: str) -> Dict[str, List[str]]:
        """Extract financial data and metrics"""
        financial_data = {}

        patterns = {
            'currencies_amounts': [
                r'\$\s*\d+[.,]?\d*\s*(millones?|billones?|mil)?',
                r'USD\s*\d+[.,]?\d*\s*(millones?|billones?)?',
                r'COP\s*\d+[.,]?\d*\s*(millones?|billones?)?'
            ],
            'percentages': [
                r'\d+[.,]?\d*\s*%',
                r'\d+[.,]?\d*\s*por\s*ciento'
            ],
            'market_metrics': [
                r'\b(capitalización|valoración).*\$\s*\d+[.,]?\d*',
                r'\b(ingresos|ventas|utilidades).*\$\s*\d+[.,]?\d*',
                r'\b(EBITDA|margen|rentabilidad).*\d+[.,]?\d*\s*%'
            ],
            'stock_data': [
                r'\b(acción|título).*\$\s*\d+[.,]?\d*',
                r'\b(precio objetivo|target price).*\$\s*\d+[.,]?\d*'
            ],
            'growth_rates': [
                r'\b(crecimiento|aumento|incremento).*\d+[.,]?\d*\s*%',
                r'\b(caída|disminución|reducción).*\d+[.,]?\d*\s*%'
            ]
        }

        for data_type, pattern_list in patterns.items():
            found = set()
            for pattern in pattern_list:
                matches = re.findall(pattern, text, re.IGNORECASE)
                found.update(matches)
            if found:
                financial_data[data_type] = list(found)

        return financial_data

    def _classify_business_content(self, content: str, url: str) -> str:
        """Classify the type of business content"""
        content_lower = content.lower()

        # Company analysis
        company_indicators = ['empresa', 'compañía', 'corporación', 'resultados', 'balance', 'utilidades']
        if any(indicator in content_lower for indicator in company_indicators) or '/empresas/' in url:
            return 'company_analysis'

        # Market analysis
        market_indicators = ['mercado', 'sector', 'industria', 'competencia', 'participación']
        if any(indicator in content_lower for indicator in market_indicators):
            return 'market_analysis'

        # Investment content
        investment_indicators = ['inversión', 'inversionista', 'fondo', 'portafolio', 'retorno']
        if any(indicator in content_lower for indicator in investment_indicators) or '/inversion/' in url:
            return 'investment'

        # Economic analysis
        economic_indicators = ['economía', 'pib', 'inflación', 'banco central', 'política monetaria']
        if any(indicator in content_lower for indicator in economic_indicators) or '/economia/' in url:
            return 'economic_analysis'

        # Entrepreneurship
        entrepreneur_indicators = ['emprendimiento', 'startup', 'innovación', 'tecnología', 'disrupción']
        if any(indicator in content_lower for indicator in entrepreneur_indicators) or '/emprendimiento/' in url:
            return 'entrepreneurship'

        # Financial markets
        finance_indicators = ['bolsa', 'acciones', 'bonos', 'divisas', 'commodities']
        if any(indicator in content_lower for indicator in finance_indicators) or '/finanzas/' in url:
            return 'financial_markets'

        return 'business_general'

    def _calculate_magazine_difficulty(self, text: str) -> float:
        """Calculate difficulty for business magazine content"""
        if not text:
            return 4.0  # Business magazines are typically complex

        words = text.split()
        sentences = re.split(r'[.!?]+', text)

        # Metrics
        avg_sentence_length = len(words) / max(len(sentences), 1)
        avg_word_length = sum(len(word) for word in words) / max(len(words), 1)

        # Business magazine complexity factors
        financial_jargon = len(re.findall(r'\b(EBITDA|ROE|ROA|ROI|WACC|DCF|NPV|CAPEX|OPEX)\b', text, re.IGNORECASE))
        business_terms = len(re.findall(r'\b(sinergia|disrupción|escalabilidad|monetización|valuación)\b', text, re.IGNORECASE))
        numbers_metrics = len(re.findall(r'\d+[.,]?\d*\s*%|\$\s*\d+|USD\s*\d+|COP\s*\d+', text))
        technical_acronyms = len(re.findall(r'\b[A-Z]{2,}\b', text))

        # Start higher for magazine content
        difficulty = 3.8

        # Sentence complexity
        if avg_sentence_length > 22:
            difficulty += 0.4
        if avg_sentence_length > 32:
            difficulty += 0.5

        # Word complexity
        if avg_word_length > 7.5:
            difficulty += 0.4

        # Business complexity factors
        difficulty += min(1.0, financial_jargon / 20)
        difficulty += min(0.8, business_terms / 30)
        difficulty += min(0.6, numbers_metrics / 60)
        difficulty += min(0.4, technical_acronyms / 15)

        return min(5.0, difficulty)

    def _get_difficulty_level(self, score: float) -> str:
        """Convert difficulty score to level for business magazine content"""
        if score < 3.5:
            return 'Intermedio-Avanzado'
        elif score < 4.2:
            return 'Avanzado'
        elif score < 4.7:
            return 'Avanzado-Experto'
        else:
            return 'Experto'

    def _parse_spanish_date(self, date_text: str) -> Optional[datetime]:
        """Parse Spanish date formats"""
        months = {
            'enero': 1, 'febrero': 2, 'marzo': 3, 'abril': 4,
            'mayo': 5, 'junio': 6, 'julio': 7, 'agosto': 8,
            'septiembre': 9, 'octubre': 10, 'noviembre': 11, 'diciembre': 12,
            'ene': 1, 'feb': 2, 'mar': 3, 'abr': 4, 'may': 5, 'jun': 6,
            'jul': 7, 'ago': 8, 'sep': 9, 'oct': 10, 'nov': 11, 'dic': 12
        }

        # Date patterns
        patterns = [
            r'(\d{1,2})\s+de\s+(\w+)\s+de\s+(\d{4})',  # "15 de enero de 2024"
            r'(\w+)\s+(\d{1,2}),?\s+(\d{4})',          # "enero 15, 2024"
            r'(\d{1,2})/(\d{1,2})/(\d{4})',            # "15/01/2024"
            r'(\d{4})-(\d{2})-(\d{2})'                 # "2024-01-15"
        ]

        for pattern in patterns:
            match = re.search(pattern, date_text)
            if match:
                try:
                    if '/' in pattern:
                        day, month, year = match.groups()
                        return datetime(int(year), int(month), int(day))
                    elif '-' in pattern:
                        year, month, day = match.groups()
                        return datetime(int(year), int(month), int(day))
                    else:
                        if 'de' in pattern:
                            day, month_name, year = match.groups()
                            month = months.get(month_name.lower())
                        else:
                            month_name, day, year = match.groups()
                            month = months.get(month_name.lower())

                        if month:
                            return datetime(int(year), month, int(day))
                except:
                    continue

        return None

    def _clean_text(self, text: str) -> str:
        """Clean and normalize text"""
        if not text:
            return ""

        # Remove extra whitespace
        text = ' '.join(text.split())
        text = re.sub(r'[\r\n\t]+', ' ', text)
        text = re.sub(r'\s+', ' ', text)

        return text.strip()